...
- **⚠ 缺陷 3 立案（08-03，長 run regime）**：w̄_s 系統性負漂 ~−0.15%/s——32-cycle 長 run
  注入臂 12/12 低於真值（mean −5.35%）、**無注入臂 4/4 同漂（mean −4.1%）⇒ 與注入無關**；
  三臂消融排除 ridge（釘 (b,p) 照卡 z 3.12）/echo 元兇（oracle ctrl 更糟）/更新飢餓；
  主嫌 = osc 窗二階整流；放大條件 = Q77=0 無恢復＋hold 不可觀＋ā 永不重錨。
  audit 另揭 **y 軸 meas_noise 0.00057 vs 規格 5.7e-5 差 10×**（07-14 起全家族）。
  **4.8s 場景審計結論不變**（缺陷 4.8s 內只累積 ~−0.7%）；c5 校正鏈（4.8s runs）不受影響；
  長 run 辨識軌跡路線修復前不可用。修復 = 推導整流項（驗收判準已立）。
  詳 memory `project-formB-longrun-ws-drift-defect-2026-08-03`
