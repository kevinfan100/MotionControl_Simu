...
| 04 | 7-state EKF MATLAB implementation (Phase 4a-c + tests) | 2026-04-29 | IN-PROGRESS | model/controller/motion_control_law_eq17_7state.m, test_script/unit_tests/ | Phase 4a/4b/4c 三個 .m 檔 + 三個 unit test .m 檔全部完成（純 MATLAB，未動 Simulink）；checkcode 0 issues；pending 使用者本地跑 test_*.m 驗證；Phase 4d/4e/4f（Simulink 整合 + dual-track equivalence）後置 |
