...
\[ e_{y_1}[k] = y_1[k] - \hat y_1[k] = e_{\delta x_1}[k] + n_x[k], \qquad e_{y_2}[k] = y_2[k] - \hat y_2[k] = e_{a_x}[k] + r_2[k] \]
\[
\begin{aligned}
...
\textbf{True-signal dynamics} (no estimate; $x_{ram}=-\delta x$, $\delta x[k+1]=\lc\,\delta x[k]-\varepsilon[k]$):
\[ a_{x_{ram}}[k+1] = -a'_x\,\delta x[k+1] = -a'_x\big(\lc\,\delta x[k]-\varepsilon[k]\big) = \lc\, a_{x_{ram}}[k] + a'_x\,\varepsilon[k] \]
\[ \Delta a_{ram}[k] = a_{x_{ram}}[k+1]-a_{x_{ram}}[k] = \underbrace{(\lc-1)\,a_{x_{ram}}[k]}_{\text{reversion}} + \underbrace{a'_x\,\varepsilon[k]}_{\text{white}} \]

\textbf{True gain state} ($a_x = a_{x_{det}} + a_{x_{ram}}$, $a_{x_{det}}[k+1]=a_{x_{det}}[k]+\Delta a_x[k]$):
\[
\begin{aligned}
...
\textbf{Estimator} (predict reverts to $a_{x_{det}}$):
\[ \hat a_x[k+1] = \lc\,\hat a_x[k] + (1-\lc)\,a_{x_{det}}[k] + \Delta a_x[k] + \ell_{4\cdot}[k]\,e_y[k] \]
\[
\begin{aligned}
e_{a_x}[k+1] &= a_x[k+1] - \hat a_x[k+1] \\
&= \lc\big(a_x[k]-\hat a_x[k]\big) + a'_x\,\varepsilon[k] - \ell_{4\cdot}[k]\,e_y[k] \\
&= \lc\, e_{a_x}[k] + a'_x\,\varepsilon[k] - \ell_{4\cdot}[k]\,e_y[k]
\end{aligned}
\]
...
\textbf{Impl.}: predict and $q_4$ use $\hat a_x$ with $a'_x$ at $\bar h_{\mathrm{meas}}$ ($p_m$); reversion target $a_{x_{det}}$ from $p_d$.
