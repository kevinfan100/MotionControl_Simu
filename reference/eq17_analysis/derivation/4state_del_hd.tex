...
\medskip
\textit{Remark --- $x_{ram}$ is the filtered state, so $q_4$ is small.}\;
With $h = h_d - \delta x$ and negligible deterministic tracking error (the feed-forward
and pole $\lc$ make $\delta x$ purely thermal; cf.\ the $z$ tracking-variance figure),
$x_{ram} = -\delta x = -\delta x_3$ is estimated by the filter. Then
$\delta a_{ram} = -a'_x\,\Delta\delta x$ is observable, and feeding its predictable part forward,
\[ \hat a_x[k+1] = \hat a_x[k] + \delta a_x[k] - a'_x\,\Delta\delta\hat x_3[k], \]
leaves only the residual $q_4 = -a'_x\,\Delta e_{\delta x_3}[k]$, with
$\mathrm{Var}(q_4) = a'^2_x\,\mathrm{Var}(\Delta e_{\delta x_3}) \ll \mathrm{Var}(\delta a_{ram})$.
(The induced $a_x$--$\delta x_3$ coupling in $\mathbf F_e$ row 4 is deferred.)
...
4-state vs 5-state, per-axis EKF, seed ensemble; near-wall 1\,Hz oscillation ($h_{init}=50\,\mu$m, trough $\bar h = 1.2$).

\medskip
\noindent\begin{minipage}{\linewidth}\centering
\includegraphics[width=0.78\textwidth]{figures/fig3_trajectory_osc1hz.png}\\[0.4ex]
{\small\itshape Desired $p_d$ and actual $p_{true}$ ($z$).}
\end{minipage}
...
\medskip
\noindent\begin{minipage}{\linewidth}\centering
\includegraphics[width=0.82\textwidth]{figures/fig_track_var_osc1hz.png}\\[0.4ex]
{\small\itshape $z$ tracking-error variance (across seeds), 4-state / 5-state, and the closed-loop theory $C_{\delta x}\,4k_BT\,a_z$ ($C_{\delta x}=2+1/(1-\lambda_c^2)$).}
\end{minipage}
