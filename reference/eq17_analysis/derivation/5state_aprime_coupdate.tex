...
\medskip
\hd{8.1 Why $\hat a_z$ jitters, and how to reduce it}{a leak-free gain integrator anchored by a noisy $a_{xm}$}

The gain state $a_h$ is a \emph{leak-free integrator} ($F_e(4,4)\approx1$, no
reversion): its only anchor is $y_2=a_{xm}$, itself a finite-window variance
inversion with relative std $0.37$--$0.54$; and its predict injects thermal noise
through $(1-\lc)\aph\,\dxh_3$. So even when the true gain is constant, $\hat a_z$
wanders $\sim5\%$ around it:
\begin{center}\small
\begin{tabular}{lccc}
\toprule
segment & true $a_z$ (nm/pN) & $\langle|\hat a_z-a_z|\rangle$ (nm/pN) & rel. \\
\midrule
hold (far wall) & $13.96$ & $0.73$ & $5.2\%$ \\
descent & $12.29$ & $0.71$ & $5.8\%$ \\
osc (near wall) & $9.23$ & $0.51$ & $5.5\%$ \\
hold (near wall) & $6.92$ & $0.30$ & $4.3\%$ \\
\bottomrule
\end{tabular}
\end{center}
The absolute wander tracks the gain (largest far from the wall, where $a_{xm}$'s
noise $\propto a$ is larger). The balance is a $Q_{44}/R_2$ tension: raising
$Q_{44}$ tracks a changing gain (motion) but amplifies the $a_{xm}$ noise;
lowering it smooths but lags. With $a_{xm}$ the \emph{only} gain measurement,
smooth-and-responsive cannot both hold --- the residual $\sim5\%$ is a CRLB floor.

\textbf{How to reduce it.} (i) Give the gain a reversion instead of a free
integrator: the \texttt{kfmeas} $a_d$-state (deterministic predict
$a_d+\aph\Delta h_d$) cuts the \emph{predict-side} wander $\sim30\times$
($2.74\%\to0.09\%$), though the measurement-anchored jitter remains. (ii) Lengthen
the $a_{xm}$ window (smaller $a_{cov}$) to quiet the ruler, at the cost of lag.
(iii) Break the floor only with \emph{more information} --- an independent gain
channel or trajectory dither; tuning $Q_{44}/R_2$ alone merely slides along the
smooth-vs-responsive trade-off.

\medskip
\hd{8.2 If accurate $\hat a_z$ is the goal}{a charter-compliant, derivable optimization path}

Under the charter ($c(\bar h)$ unknown; only $a_{nom},R,T_s$, the wall position,
and the commanded trajectory are usable), two legal levers remain:

\emph{(a) Structural --- free and already derived.} Adopt the \texttt{kfmeas}
$a_d$-state: its predict $a_d+\aph\,\Delta h_d$ uses \emph{only} the commanded
$\Delta h_d$ and the estimated $\aph$, assumes no $c(\bar h)$, and is the validated
$\sim30\times$ predict-wander reduction ($2.74\%\!\to\!0.09\%$). This removes the
leak-free random walk at zero information cost.

\emph{(b) Information --- to pass the CRLB floor.} The measurement-anchored jitter
is set by $a_{xm}$ being the \emph{sole} gain channel; it falls only with an
independent, $c$-free channel (a derived deterministic residual, unbiased slope
$0.90$--$0.94$) or trajectory dither (the trajectory is ours to design). Both are
derivable and assume no wall-function form; tuning $a_{cov}/Q_{44}/R_2$ only trades
jitter for lag and cannot pass the floor.
