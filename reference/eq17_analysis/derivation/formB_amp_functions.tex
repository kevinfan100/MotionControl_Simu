...
%          nothing is fitted.  Single seed (7); the 12-seed means differ from
...
\begin{equation*}
b_{\mathrm{eff}} = \text{page 1};
\qquad
\sup_{\text{envelope}}\bigl|b_{\mathrm{eff}} - \tfrac{9}{8}\bigr|
  = 0.0157\ \text{(B)},\ \ 0.0708\ \text{(C)}
\end{equation*}

\begin{equation*}
\text{descent peak},\ \text{osc RMS},\ \text{final hold}\ [\%]
\qquad
\text{B}:\ 0.95,\ 0.26,\ -0.60
\qquad
\text{C}:\ 5.47,\ 1.87,\ -3.52
\end{equation*}}{formB_cl_perp.png}

% ------------------------------------------- step 7: closed loop, para
\fpage{%
\begin{align*}
\text{truth} &= c_\parallel
& b_0 &= \tfrac{9}{16}\ \text{(both)}
& p &= 1,\ \ \bar{w}_s = 1 \\
\hat{b}\ \text{estimated} & & \sqrt{P[0]} &= 0.1880\ \text{(B)},\ \ 0.0072\ \text{(C)}
& & \text{one seed}
\end{align*}

\begin{equation*}
\sup_{\text{envelope}}\bigl|b_{\mathrm{eff}} - \tfrac{9}{16}\bigr|
  = 0.1880\ \text{(B)},\ \ 0.0072\ \text{(C)}
\qquad
\text{swing}\ = 0.1769,\ \ 0.0069
\end{equation*}

\begin{equation*}
\text{descent peak},\ \text{osc RMS},\ \text{final hold}\ [\%]
\qquad
\text{B}:\ 5.81,\ 2.00,\ -3.83
\qquad
\text{C}:\ 0.91,\ 0.44,\ -0.58
\end{equation*}

\begin{equation*}
c_\parallel\ \text{substituted into the wall-normal channel};
\qquad
c_\perp\ \text{has a pole at contact},\ \ c_\parallel\ \text{has none}\ (3.084)
