...
%
%          PAGES 6-7 (2026-08-09) close the loop on page 1.  Page 1's
...
%          nothing is fitted.  p.7 KEEPS p.6's anchor 9/8 -- a controller that
...
%          on c_para (0.7505 B, 0.5697 C).  With p.6's 0.0157/0.0708 instead,
...
%          Figures: test_script/scratch/plot_formB_closedloop_pages.m
%          Arms:    test_script/scratch/run_formB_BC_two_boundaries.m  (p.6)
%                   test_script/scratch/run_formB_blind_para.m         (p.7)
%          Acceptance for the amplitude arm: verify_formB_amp_tie.m (7/7 PASS).
%          HOW THE RED CURVES RELATE TO PAGE 1 (checked, not assumed):
...
%              pp.6-7 are in TIME, and the trajectory maps time to gap
...
%              is never visited;
%            p.7's truth is NOT on page 1 at all -- page 1 is c_perp only, so p.6's
%              red curves are page 1's, reparametrised, while p.7's are c_para.
...
%          pages 3-5 the constant, pages 6-7 the estimate of it.
...
%          test_script/scratch/plot_formB_closedloop_pages.m  (pp.6-7)
