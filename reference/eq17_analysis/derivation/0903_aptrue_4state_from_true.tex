...
%   at the wall after the descent (open loop; the y2 leg removes >= 95% of it in motion). Section 12 = the covariance
%   weight kappa of the self-referential law (0.5 calibrated on the b_true arm; production stays at 1, see S12). Last three pages: the
...
%   fe44_Aa_scale stays 1 (Section 12); acceptance test_script/integration/verify_formC_b_production_defaults.m.
...
\subsection*{The container of the $b'_{\mathrm{true}}\,\dW$ line: $\tilde{Q}_{55}$ proportional to the path (09-07)}

($e_b$ is driven every step by $b'_{\mathrm{true}}(\bw)\,\dW$, the change of the exact law factor along the path; the constant model
has no term for it and $\tilde{Q}_{55} = 0$ lets $P_{55}$ collapse below $|e_b|$ -- measured 09-07 on the estimated-$\hat b$ arm with
the seed-at-truth $P_{44}[0]$: $\sqrt{P_{55}} \to 0.0065$, hold $-0.0106 \pm 0.0021$ canon, $\hat b$ pinned at $0.96$ against
$b_{\mathrm{true}}(\bw_{\mathrm{hold}}) = 0.928$. $\Delta b := \sup_{\text{env}} b_{\mathrm{true}} - \inf_{\text{env}} b_{\mathrm{true}}$,
$W := \bw_{hi} - \bw_{lo}$, both from the envelope the driver already sweeps; the sign of $b'_{\mathrm{true}}$ is not known to the filter)
\begin{align*}
e_b[k+1] &= e_b[k] + b'_{\mathrm{true}}\bigl(\bw[k]\bigr)\,\dW[k] - \ell_{51}[k+1]\,e_{y_1}[k+1] - \ell_{52}[k+1]\,e_{y_2}[k+1] \\
\sum_{\text{one traverse}} \bigl|\,b'_{\mathrm{true}}\bigl(\bw[k]\bigr)\,\dW[k]\,\bigr| &\le \Delta b \\
\tilde{Q}_{55}[k] &= \frac{\Delta b^{2}}{W}\,\bigl|\dWh[k]\bigr| \\
\sum_{\text{one traverse}} \tilde{Q}_{55}[k] &= \Delta b^{2} \\
P_{55}^{-}[k+1] &= P_{55}[k] + \tilde{Q}_{55}[k] \qquad (\text{row and column 5 of } \tilde{F}_e P \tilde{F}_e^{\top} \text{ unchanged}) \\
\sqrt{P_{55}}\ \text{after a path } s W,\ 0 \le s \le 1,\ \text{from } P_{55} = 0 &= \Delta b\,\sqrt{s} \ \ge\ \Delta b\,s
  \ \ge\ \Bigl|\sum b'_{\mathrm{true}}\dW\Bigr| \qquad \text{only if } \max\bigl|b'_{\mathrm{true}}\bigr| \le \Delta b / W
  \quad (\text{uniform-rate condition; the real curve exceeds it } 10\times) \\
\tilde{Q}_{55} &= 0 \ \text{in a hold } (\dWh = 0) , \qquad \Delta b = 0 \ \text{on the b}_{\mathrm{true}}\text{ arm (Section 10 recovered)}
\end{align*}

(no free number: $\Delta b$ and $W$ are the envelope's; $\Delta b^{2}/W$ is passed by the driver as q55\_per\_R, the controller multiplies by
$|\dWh|$ each step. Code: ctrl\_const.q55\_path (default false). Acceptance pre-registered in test\_script/scratch/run\_ladder\_endpoints.m
arms bhq5 / btq5 / bhq5s: the estimated-$\hat b$ arm with the seed-at-truth $P_{44}[0]$ must reach hold $|{\rm mean}| \le 1$ SEM and a
near-wall mean $|\cdot| < 0.01$ (from $-0.0106$ and $-0.04$), $\sqrt{P_{55}}$ at the end $\ge 0.02$, $\hat b$ in hold within $\sqrt{P_{55}}$
of $b_{\mathrm{true}}(\bw_{\mathrm{hold}})$, near-wall $\sigma_{\mathrm{seed}} \le 0.015$; the b$_{\mathrm{true}}$ arm bit-identical;
production ($P_{44}[0]$ as is) with the term: hold within 1 SEM, $\sigma_{\mathrm{seed}}$ not above $0.032$)

Measured (10 seeds, four blocks, $\kappa = 1$; Meng [canon]; near-wall window Meng 7--10 s, canon 1.0--1.5 s; $\ba$ units)
\begin{align*}
\text{seed-at-truth } P_{44}[0],\ \tilde Q_{55} = 0:&\quad \text{hold } -0.0024 \pm 0.0021\ [-0.0106 \pm 0.0021],\quad
  \text{near wall } -0.019\ \sigma\,0.017\ [-0.012\ \sigma\,0.009],\quad \hat b\ 0.940\ (\sqrt{P_{55}}\ 0.010)\ [0.973\ (0.006)] \\
\text{seed-at-truth } P_{44}[0],\ \tilde Q_{55}\ \text{on}:&\quad \text{hold } -0.0006 \pm 0.0024\ [+0.0059 \pm 0.0031],\quad
  \text{near wall } -0.013\ \sigma\,0.039\ [-0.003\ \sigma\,0.023],\quad \hat b\ 0.875\ (0.066)\ [0.867\ (0.039)] \\
\text{production } P_{44}[0],\ \tilde Q_{55}\ \text{on}:&\quad \text{hold } -0.0006 \pm 0.0024\ [+0.0060 \pm 0.0031],\quad
  \text{near wall } -0.012\ \sigma\,0.039\ [-0.003\ \sigma\,0.023] \\
\text{b}_{\mathrm{true}}\text{ arm with the term}:&\quad \text{bit-identical to Section 10}, \qquad b_{\mathrm{true}}(\bw_{\mathrm{hold}}) = 0.924\ [0.928]
\end{align*}

(STATUS 09-07: REFUTED as written. The container removes the hold bias on Meng and halves canon's near-wall mean, but doubles the
near-wall spread (pre-registered lines 2 and 4 fail), $\sqrt{P_{55}}$ over-covers $|e_b|$ by $2\times$ -- the $\sqrt{s}$ against $s$ of the
line above, by construction -- and $\hat b$ drifts below the curve ($0.87$ against $0.92$--$0.93$) while the extra freedom is filled by
gain transients, not by $b_{\mathrm{true}}$ (the 08-17 information bound). With the term on, the seed-at-truth and the production
$P_{44}[0]$ give the same run: the container dominates $P_{44}$ through $\tilde F_e(4,5)$. What the three estimated-$\hat b$ rows share is a
near-wall MEAN of $-0.01$ to $-0.02$ that no covariance container removes -- it is the mean model error of a constant $b$
(the $\mu$ of this section), and the b$_{\mathrm{true}}$ arm, whose mean model follows the curve, has none. A covariance term is the
wrong category for a coherent bias; the mean model is. Flag kept, default false.
Independent verification 09-07 (sympy / numeric, three interpolants of the real $b_{\mathrm{true}}$ through $0.883$ at $6.67$, $0.867$ at $2.19$,
$0.928$ at $1.10$): items 1 and 4 exact; the honesty line needs the uniform-rate condition written above -- with it dropped a curve that
spends its whole $\Delta b$ early is uncovered; on the real curve the container covers every DESCENT (worst $|e_b|/\sqrt{P_{55}} = 0.74$,
at the wall end) but not an ASCENT ($2.5\times$ Meng, $4.8\times$ canon, $1.55\times$ the outward oscillation leg): the steep part of
$b_{\mathrm{true}}$ sits at the wall, late on a descent and early on an ascent. A run that never resets $P_{55}$ carries the descent's
margin through the ascent with $\hat b$ frozen, but the filter shrinks $P_{55}$ at every update, so that margin is not guaranteed --
consistent with the measured $\hat b$ drift on the oscillating canon profile.)

\subsection*{E2 (09-07): the additive gain disturbance $\delta a$ alongside $\hat b$ -- paper gates 1--3}

(slot 6 of the state, the formC\_dist 'gain' placement: $\hat{\ba}_w[k+1]$ gains $+\delta\hat a[k]$ outside the law bracket, $\delta\hat a[k+1] = \delta\hat a[k]$,
$\sqrt{P_{66}[0]}$ = the driver's $S3(b)$ closed-form supremum, $\tilde Q_{66} = 0$, seed $0$; purpose: the innovations that the
estimated-$\hat b$ arm with a small $P_{44}[0]$ charges to $\hat b$ -- canon $+0.057$ in the descent, $99\%$ through $\ell_{51}$; Meng $+0.039$
through $P_{54}H_{24}$ -- get a level-type state to go to; motivation and the E1 discriminator in the memory file of 09-07)

Gate 1, the two columns of each slot
\begin{align*}
\tilde F_e(4,6) &= 1 , \qquad H_{26} = -d \qquad (\text{structural constants: non-zero in a hold and at a turning point}) \\
\tilde F_e(4,5) &= \frac{\ap\,\dWh}{\hat b} , \qquad H_{25} = -\frac{\partial\ap}{\partial\hat b}\,\nabla_d\bw_d \qquad (\text{both carry a displacement})
\end{align*}

Gate 2, time signature of an error: $e_{\delta a}$ = a ramp of $\hat{\ba}_w$ with slope $e_{\delta a}$ per step plus the direct readout $-d\,e_{\delta a}$;
$e_b$ = a ramp with slope $\ap\dWh\,e_b/\hat b$ per step plus the readout $H_{25}e_b$: the same shape wherever $\ap\dWh$ is constant.

Gate 3, the three-state subsystem $[\ba_w,\ \delta a,\ b]$ seen by $y_2$ ($\nabla_d\bw_d = 0$ for the moment)
\begin{align*}
F_3 &= \begin{bmatrix} 1 & 1 & \ap\dWh \\ 0 & 1 & 0 \\ 0 & 0 & 1 \end{bmatrix} , \qquad
H_3 = \begin{bmatrix} 1 & -d & 0 \end{bmatrix} \\
O_3 &= \begin{bmatrix} H_3 \\ H_3 F_3[k] \\ H_3 F_3[k+1] F_3[k] \end{bmatrix}
   = \begin{bmatrix} 1 & -d & 0 \\ 1 & 1-d & \ap\dWh[k] \\ 1 & 2-d & \ap\dWh[k] + \ap\dWh[k+1] \end{bmatrix} \\
\det O_3 &= \ap\dWh[k+1] - \ap\dWh[k] \\
\det O_3 = 0 &\iff \ap\dWh \ \text{constant across the window} \qquad (\text{uniform far-field descent: } \delta a \text{ and } b \text{ alias})
\end{align*}

(they separate where $\ap$ changes -- near the wall -- or where $\dWh$ changes -- turning points and the entry to a hold; in a hold
$\delta a$ stays observable through $H_{26} = -d$ and the ramp, $b$ does not (Section 11). Gates 4--5: test\_script/scratch/verify\_obs\_da\_slot.m,
negative control slot 7. Code: ctrl\_const.da\_slot, default false; arms bhd / btd / bhatd in run\_ladder\_endpoints.m)

Gates 4--5 measured (seed 7, window 500 steps unless stated; best CRLB / prior; test\_script/scratch/verify\_obs\_da\_slot.m)
\begin{align*}
\text{canon, } \delta a \text{ free}:&\quad \text{rank } 8/8,\ \text{negative control } \infty\ 15/15;\quad
  \ba_w\ 0.060,\ b\ 0.047,\ \delta a\ 0.0044 \quad (\text{without } \delta a:\ \ba_w\ 0.017,\ b\ 0.0088) \\
\text{Meng, } \delta a \text{ free}:&\quad \text{rank } 8/8,\ \infty\ 40/40;\quad b\ 2.88\ \text{FAIL at 500 steps},\ 0.17\ \text{at 2000},\ 0.040\ \text{at 4000};\quad \delta a\ 0.53
  \quad (\text{without } \delta a:\ b\ 0.54) \\
\text{gate-3 condition seen:}&\quad \text{on the slow ramp a 500-step window spans } 0.13\,R,\ \ap\dWh \text{ hardly changes} \Rightarrow \delta a \text{ and } b \text{ alias; a window over more path separates them}
\end{align*}

Arms (10 seeds; near-wall window Meng 7--10 s, canon 1.0--1.5 s; $\ba$ units; Meng [canon])
\begin{align*}
\text{bhp0 (estimated } \hat b,\ \text{seed-at-truth } P_{44}[0]):&\quad \text{hold } -0.0024 \pm 0.0021\ [-0.0106 \pm 0.0021],\
  \text{near wall } -0.019\ \sigma\,0.017\ [-0.012\ \sigma\,0.009],\ \hat b\ 0.940\ [0.973] \\
\text{bhd (the same} + \delta a):&\quad \text{hold } -0.0006 \pm 0.0023\ [+0.0035 \pm 0.0017],\
  \text{near wall } -0.010\ \sigma\,\mathbf{0.046}\ [-0.0002\ \sigma\,\mathbf{0.038}],\ \hat b\ 0.885\ (\sqrt{P_{55}}\ 0.033)\ [0.869\ (0.028)],\
  \delta\hat a \to 0,\ \sqrt{P_{66}} \to 0 \\
\text{btd (b}_{\mathrm{true}} + \delta a):&\quad \text{hold } +0.0002 \pm 0.0023\ [+0.0017 \pm 0.0017],\ \sigma\ 0.046\ [0.038]\ \text{against btp0's } 0.003\ [0.001] \\
\text{bhatd (production } P_{44}[0] + \delta a):&\quad \text{identical to bhd on both trajectories}
\end{align*}

(STATUS 09-07: the slot does what it was asked -- $\hat b$ is no longer pulled to the wall value and locked ($0.87$--$0.89$ with
$\sqrt{P_{55}}$ open), the canon hold bias $-0.0106$ becomes $+0.0035$, Meng's $-0.0006$ -- but the pre-registered spread line fails
by $3$--$4\times$: $\delta a$ is a per-step INCREMENT with the $S3(b)$ prior $3\times10^{-4}$ [$1.6\times10^{-5}$] per step, which integrates
to a gain uncertainty of order $0.1$--$0.5$ over the far-field part of the descent; the filter lets $\hat{\ba}_w$ walk until $y_2$ pins
$\delta a$ (figure: seed 7 steps of $\pm 0.05$ between $2$ and $7$ s on Meng, $0.2$--$1.3$ s on canon), and the b$_{\mathrm{true}}$ arm pays
the same $0.04$. The near-wall MEAN of the estimated-$\hat b$ arms is unchanged on Meng ($-0.010$): as with $\tilde Q_{55}$, a state
that carries model error does not remove the mean of a constant-$b$ law. REFUTED as a production candidate; kept as an instrument,
default false. What survives from E1/E2: the innovations of the estimated-$\hat b$ arm with a small $P_{44}[0]$ need a home -- a drift
state gives them one at the price of far-field spread, a large $P_{44}[0]$ (production) gives them one at the price of near-wall
spread, and neither touches the mid-band mean.)


\clearpage
\section{Covariance propagation of the self-referential law: the weight $\kappa$}

(arms whose slope reads $\hat{\ba}_w$, Sections 10--11; $A := \partial\ap/\partial\hat{\ba}_w = -2\hat{b}(1-\hat{\ba}_w)$;
$z := e_{\ba_w} + \ap\,e_{\delta\bw_3}$ the law-slaved combination of Section 2 of the update-half document;
everything else as in Sections 1--11)

Linear error dynamics in the $(e_{\delta\bw_3},\ z)$ basis (from Section 10, $e_{\ba_w} = z - \ap e_{\delta\bw_3}$, $\ap[k+1] = \ap + \app\dWh$, $\app = A\,\ap$)
\begin{align*}
z[k+1] &= \bigl(1 + A\,\dWh\bigr)\,z[k] - A\,\dWh\,\ap\,\eD[k] \\
e_{\delta\bw_3}[k+1] &= \bigl(\lambda_c + F_{dw}\,\ap\bigr)\,e_{\delta\bw_3}[k] + (1-\lambda_c)\,e_{\delta\bw_1}[k] - F_{dw}\,z[k] - \et[k] \\
\ell_{41} + \ap\,\ell_{31} &= 0 \iff \mathrm{Cov}\bigl(z,\ e_{\delta\bw_1}\bigr) = 0
\end{align*}

(the amplification $1 + A\dWh$ acts on $z$; the position noise enters the gain through the law as $\ap\,\et$ and is
removed by the $y_1$ update exactly when the second line holds -- the a$'_{\mathrm{true}}$ arm keeps it in every segment,
$|\ell_{41} + \ap\ell_{31}| < 0.002$)

Measured (b$_{\mathrm{true}}$ arm, four blocks, canon $1.0$--$1.5$ s, 10--30 seeds; Meng $7$--$10$ s in brackets)
\begin{align*}
\kappa = 1:\quad P_{44} &\ \text{grows } \times 16 \text{ along the descent} \\
\ell_{41} &= +0.036 , \qquad -\ap\ell_{31} = -0.157 , \qquad \ell_{41} + \ap\ell_{31} = +0.19\ [+0.039] \\
\sigma(\text{predict}) &= 0.025 , \qquad \sigma(y_1\ \text{leg}) = 0.009 , \qquad \text{corr} = -0.40
  \qquad (\text{a}'_{\mathrm{true}}\text{ arm: } 0.018,\ 0.017,\ -0.99) \\
\sigma_{\mathrm{seed}}(\hat{\ba}_w - \ba_w) &= 0.026\ [0.032] \text{ at the worst instant} , \qquad
  \sigma_{\mathrm{seed}}/\sqrt{P_{44}} = 0.66\ [0.81] \\
\kappa = 0:\quad \ell_{41} + \ap\ell_{31} &= 0.002\ [0.001] , \qquad \sigma_{\mathrm{seed}} = 0.0045\ [0.0079] \\
\sigma_{\mathrm{seed}}/\sqrt{P_{44}} &= 1.24\ [1.60] \text{ (fast)} , \qquad 1.75\ [2.07] \text{ (hold)}
\end{align*}

Weighted propagation (covariance only; the state predict, the mean term and $H_2$ keep $A$)
\begin{align*}
\tilde{F}_e(4,4) &= 1 + F_{dw}\,\ap + \kappa\,A\,\dWh , \qquad \kappa \in [0, 1]
\end{align*}

Calibration (seeds 1:10, both trajectories; honesty $= \sigma_{\mathrm{seed}}/\sqrt{P_{44}}$, geometric mean over $\{\text{fast},\ \text{hold}\}\times\{\text{canon},\ \text{Meng}\}$)
\begin{align*}
\kappa &= 0,\ 0.25,\ 0.5,\ 0.75,\ 1 \\
\text{honesty (geo.\ mean)} &= 1.64,\ 1.27,\ \mathbf{0.96},\ 0.81,\ 0.84 \\
\sigma_{\mathrm{seed}}\ \text{worst instant, canon} &= 0.0045,\ 0.0046,\ \mathbf{0.0054},\ 0.0109,\ 0.0264 \\
\sigma_{\mathrm{seed}}\ \text{worst instant, Meng} &= 0.0079,\ 0.0101,\ \mathbf{0.0158},\ 0.0249,\ 0.0323 \\
\text{hold honesty canon / Meng} &= 1.75/2.07,\ 1.68/1.25,\ \mathbf{1.19/1.06},\ 0.88/1.07,\ 0.87/1.08
\end{align*}

Validation (seeds 11:20, $\kappa = 0.5$ vs $1$)
\begin{align*}
\sigma_{\mathrm{seed}}\ \text{worst instant} &: 0.0237 \to 0.0026\ (\text{canon}),\quad 0.0272 \to 0.0082\ (\text{Meng}) \\
\text{hold } \sigma_{\mathrm{seed}} &: 0.0103 \to 0.0056,\quad 0.0091 \to 0.0081 \qquad
\text{hold honesty}: 1.43 \to 1.25,\quad 1.36 \to 1.29 \\
\text{hold level} &: +0.0022 \pm 0.0031 \to +0.0012 \pm 0.0016,\quad +0.0001 \pm 0.0027 \to +0.0004 \pm 0.0024
\end{align*}

($\kappa$ = code ctrl\_const.fe44\_Aa\_scale. STATUS: $\kappa = 0.5$ is a CALIBRATED value for the b$_{\mathrm{true}}$ arm only
(observable: the honesty ratio; calibration and judgement on disjoint seeds). It was made the production default for a few
hours on 2026-09-04 and REVERTED to $\kappa = 1$ the same night: on the arms with a constant $b$ the smaller $P_{44}$ lets the
constant-$b$ model error of Section 11 stand -- $b$ locked at $8/9$, canon, 10 seeds: hold $-0.0197 \pm 0.0007$, every seed at the
$\ba$ floor ($+0.0006$ at $\kappa = 1$); $\hat{b}$ estimated (production): hold $+0.0030$ vs $+0.0017$, worst-instant mean $-0.031$ vs
$-0.034$ (no gain), $\hat{b}$ pulled to $0.852$--$0.858$ with $\sqrt{P_{55}}$ collapsing to $0.004$ (canon). The calibration does not
transfer across arms: the term it weights competes with the $y_2$ correction of the model error, which the b$_{\mathrm{true}}$
arm does not have. What it stands in for: the split of the gain error into a free part, amplified by $1 + A\dWh$, and a
law-slaved part that follows $e_{\delta\bw_3}$ and the growth of $\ap$, which the first-order propagation treats alike; the
closed form is open, and a production-safe version must be derived (or calibrated) with the model error present.
Not part of this: the $y_2$ weight -- $\mathrm{NIS}_2 = e_{y_2}^2/S_2 = 0.27$--$0.34$ in every segment, both trajectories, both $\kappa$
(the $IF$ colour design), $\mathrm{NIS}_1 = 1.00$; nothing segment-specific to correct there)

\subsection*{Closed form: the Jacobian of the exact law step (replaces $\kappa$)}

(the predict of Section 5 with the exact step of 09-01/09-04; $\hat b$ = the constant the law uses ($b_{\mathrm{true}}(\bw)$ fed on the
b$_{\mathrm{true}}$ arm, $\hat b$ on the production arm); $\dWh = \dW_d + (1-\lambda_c)\,\delta\hat{\bw}_3 + \alpha\,(m_1+m_2) + (1-\lambda_c)\,(y_1 - \delta\hat{\bw}_1)$
the bracket the predict multiplies; $\partial\dWh/\partial\hat{\ba}_w = F_{dw}$ the loop coupling of Section 6; $A = \partial\ap/\partial\hat{\ba}_w = -2\hat b(1-\hat{\ba}_w)$)

Exact step and its partial derivatives
\begin{align*}
\hat{\ba}_w[k+1] &= 1 - \Bigl(\frac{1}{1-\hat{\ba}_w[k]} + \hat b\,\dWh\Bigr)^{-1} \\
\frac{\partial\hat{\ba}_w[k+1]}{\partial\hat{\ba}_w[k]} &= \frac{(1-\hat{\ba}_w[k+1])^{2}}{(1-\hat{\ba}_w[k])^{2}}
  = \frac{1}{\bigl(1 + \hat b\,(1-\hat{\ba}_w[k])\,\dWh\bigr)^{2}}
  = 1 + A\,\dWh + \tfrac{3}{4}\,A^{2}\dWh^{2} + \dots \\
\frac{\partial\hat{\ba}_w[k+1]}{\partial\dWh} &= \hat b\,(1-\hat{\ba}_w[k+1])^{2}
  = \frac{(1-\hat{\ba}_w[k+1])^{2}}{(1-\hat{\ba}_w[k])^{2}}\,\ap \\
\frac{\partial\hat{\ba}_w[k+1]}{\partial\hat b} &= \dWh\,(1-\hat{\ba}_w[k+1])^{2}
  = \frac{(1-\hat{\ba}_w[k+1])^{2}}{(1-\hat{\ba}_w[k])^{2}}\,\frac{\ap\,\dWh}{\hat b}
\end{align*}

Row 4 of the Jacobian of the exact step (every entry carries the same factor; Section 10 puts it on the diagonal only)
\begin{align*}
\tilde F_e(4,4) &= \frac{(1-\hat{\ba}_w[k+1])^{2}}{(1-\hat{\ba}_w[k])^{2}}\,\bigl(1 + F_{dw}\,\ap\bigr) \\
\tilde F_e(4,3) &= \frac{(1-\hat{\ba}_w[k+1])^{2}}{(1-\hat{\ba}_w[k])^{2}}\,(1-\lambda_c)\,\ap \\
\tilde F_e(4,8) = \tilde F_e(4,9) &= \frac{(1-\hat{\ba}_w[k+1])^{2}}{(1-\hat{\ba}_w[k])^{2}}\,\alpha\,\ap \\
\tilde F_e(4,5) &= \frac{(1-\hat{\ba}_w[k+1])^{2}}{(1-\hat{\ba}_w[k])^{2}}\,\frac{\ap\,\dWh}{\hat b} \qquad (\text{slot 5 free}) \\
\tilde F_e(4,1) &= -\frac{(1-\hat{\ba}_w[k+1])^{2}}{(1-\hat{\ba}_w[k])^{2}}\,(1-\lambda_c)\,\ap \qquad (\text{the } n_w \text{ input of Section 9}) \\
Q(4,4) &= \Bigl(\frac{(1-\hat{\ba}_w[k+1])^{2}}{(1-\hat{\ba}_w[k])^{2}}\Bigr)^{2}\,\ap^{2}\,Q(3,3) , \qquad
Q(3,4) = -\frac{(1-\hat{\ba}_w[k+1])^{2}}{(1-\hat{\ba}_w[k])^{2}}\,\ap\,Q(3,3)
\end{align*}

Error dynamics in the $(e_{\delta\bw_3},\ z)$ basis, noise-free, first order (row 3 as in Section 10)
\begin{align*}
\text{exact-step row:}\qquad z[k+1] &= \frac{(1-\hat{\ba}_w[k+1])^{2}}{(1-\hat{\ba}_w[k])^{2}}\,z[k] \\
\text{Euler row (Section 10):}\qquad z[k+1] &= \bigl(1 + A\,\dWh\bigr)\,z[k]
  + A\,\dWh\,\ap\,\Bigl((1-\lambda_c)\,\bigl(e_{\delta\bw_1}[k] - e_{\delta\bw_3}[k]\bigr) - F_{dw}\,e_{\ba_w}[k]\Bigr)
\end{align*}

(the Euler row is the exact Jacobian of the Euler predict $\hat{\ba}_w + \ap\dWh$ and was kept when the predict became the exact step;
per step the two rows differ by $A\dWh\times$(coupling), but the difference compounds along the descent as the diagonal does,
$\sum A\dWh = O(1)$, and the second line forces the law-slaved combination by the position errors that $y_1$ removes -- the
$\mathrm{Cov}(z, e_{\delta\bw_1}) \neq 0$ of the third line of this section. Code: ctrl\_const.jac\_exact\_step (default false);
acceptance pre-registered in test\_script/scratch/run\_jac\_exact\_arms.m; independent sympy verification 09-06: partials, row 4, $z[k+1]$ under both rows and the $Q$ row all PASS -- the Euler-row forcing above is the verifier's expression, the first draft had $\lambda_c e_{\delta\bw_3}$ in place of $-(1-\lambda_c)e_{\delta\bw_3}$)

Measured (10 seeds, four blocks, $\kappa = 1$, flag off $\to$ on; canon [Meng]; test\_script/scratch/run\_jac\_exact\_arms.m)
\begin{align*}
\text{b}_{\mathrm{true}}\ \text{arm:}\quad \sigma_{\mathrm{seed}}\ \text{worst instant} &: 0.0264 \to 0.0265\ [0.0323 \to 0.0327] , \qquad
  \ell_{41} + \ap\ell_{31}\ (\text{fast}) : +0.193 \to +0.195\ [+0.039 \to +0.040] \\
\text{hold level, paired on} - \text{off} &: +0.00016 \pm 0.00009\ [-0.00006 \pm 0.00006] \\
b = 8/9\ \text{and}\ \hat b\ \text{arms:} \quad \text{paired worst instant} &: \le 0.0002 , \qquad \text{paired hold} : \le 0.00023 \\
\text{off arms vs ladder\_endpoints (negative control)} &: \max|\Delta E| = 0
\end{align*}

(STATUS: the consistent row is the exact linearisation and is INERT -- every paired difference is at the $10^{-4}$ level on
every arm and both trajectories. The row-4 factor is therefore NOT the mechanism of the $P_{44}$ growth or of the spread;
the growth is the law's own sensitivity, $\prod (1-\hat{\ba}_w[k+1])^{2}/(1-\hat{\ba}_w[k])^{2} = \ap_{\mathrm{wall}}/\ap_{\mathrm{entry}}$,
which is the $(1-\ba)^{2}$ growth of a fixed error in $1/(1-\ba)$ and is present in truth and model alike. Refuted the same day:
$R_2/3$ (K\_var/3; the NIS$_2 = 0.3$ backlog) leaves the fast-window $\sqrt{P_{44}}$ unchanged (0.0101 $\to$ 0.0100) and makes the
hold over-confident (honesty 0.87 $\to$ 1.65 canon, 1.08 $\to$ 1.71 Meng). Flag kept, default false.)

Prior versus a real initial error (b$_{\mathrm{true}}$ arm, 10 seeds, four blocks; test\_script/scratch/run\_btrue\_prior\_vs\_offset.m)
\begin{align*}
P_{44}[0] &= \ap[0]^{2}\,P_{f,w_0}^{2} + P_{f,a,\mathrm{floor}}^{2} : \quad \sqrt{P_{44}[0]} = 0.0031\ (\text{Meng},\ 6.67\,R),\quad 0.00026\ (\text{canon},\ 22.2\,R) \\
\text{seed at truth, } \sqrt{P_{44}[0]} \to 3\times10^{-4}\ [10^{-5}],\ \kappa = 1:\quad
  \sigma_{\mathrm{seed}}\ \text{worst instant} &: 0.0323 \to 0.0051\ [0.0264 \to 0.0035] , \qquad
  \ell_{41} + \ap\ell_{31} : +0.039 \to +0.002\ [+0.193 \to +0.003] \\
  \text{hold: level } +0.0014 \pm 0.0017\ [+0.0010 \pm 0.0010],\ \text{honesty } 1.01\ [1.01] &, \qquad \text{fast honesty } 0.66\ [0.77] \\
\text{seed } +0.0031 \text{ (one prior sigma), Meng, } \kappa = 1:\quad \text{worst-instant mean} &= +0.0096 ,\quad \text{hold } +0.0007 \pm 0.0022 ,\quad \text{fast honesty } 0.88 \\
\text{seed } +0.0031,\ \kappa = 0.5:\quad \text{worst-instant mean} &= +0.0479 ,\quad \text{hold } +0.0147 \pm 0.0026 ,\quad \text{fast honesty } 0.92
\end{align*}

(VERDICT 09-06: the fast-descent $P_{44}$ of the arms that read $\ap$ at $\hat{\ba}_w$ is the prior $P_{44}[0]$ carried through the
law's own sensitivity $\prod (1-\hat{\ba}_w[k+1])^{2}/(1-\hat{\ba}_w[k])^{2}$; the realised spread follows it through the gains. In a
seed-at-truth simulation that prior is a phantom -- matching it to the start removes the spread at $\kappa = 1$ on both
trajectories, with the hold honest. With a REAL initial error of one prior sigma, $\kappa = 1$ removes it (hold $+0.0007$) and
$\kappa = 0.5$ does not ($+0.0147$, $5.6$ SEM; worst instant $+0.048$, the offset amplified $15\times$) -- the same failure as the
constant-$b$ collapse of the paragraph above. $\kappa$ is therefore REFUTED as a production quantity in every reading; the
near-wall spread is the honest price of the initial gain uncertainty under a self-referential law, and its lever is the seed
(calibration chain), not the estimator. The a$'_{\mathrm{true}}$ arm has no amplification ($A = 0$) and no such spread.)


\clearpage
\thispagestyle{empty}
\noindent\includegraphics[width=\textwidth]{figures/prod_ladder_band.png}
