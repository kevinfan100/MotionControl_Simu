...
|---|---|---|---|---|---|
| x | 3.5868e-05 | 0.827 +/- 0.019 | FAIL | 0.827 +/- 0.019 | FAIL |
| y | 3.5534e-05 | 0.820 +/- 0.013 | FAIL | 0.820 +/- 0.013 | FAIL |
| z | 3.3283e-05 | 0.788 +/- 0.015 | FAIL | 0.788 +/- 0.015 | FAIL |

## V2 — a_cov linearity

| Axis | slope | pred slope (paper) | slope/pred (paper) | pred slope (eff) | slope/pred (eff) | R^2 | pass paper | pass eff |
|---|---|---|---|---|---|---|---|---|
| x | 5.393e-04 | 8.678e-04 | 0.621 | 8.678e-04 | 0.621 | 0.9810 | FAIL | FAIL |
| y | 5.574e-04 | 8.670e-04 | 0.643 | 8.670e-04 | 0.643 | 0.9859 | FAIL | FAIL |
| z | 5.415e-04 | 8.451e-04 | 0.641 | 8.453e-04 | 0.641 | 0.9882 | FAIL | FAIL |
...
|---|---|---|---|---|---|
| x | PASS | 0.144 | 0.144 | FAIL | FAIL |
| y | FAIL | 0.156 | 0.156 | FAIL | FAIL |
| z | PASS | 0.214 | 0.214 | FAIL | FAIL |
...
|---|---|---|---|---|---|
| x | PASS | 0.173 | 0.173 | 0.000 | indistinguishable |
| y | PASS | 0.180 | 0.180 | 0.000 | indistinguishable |
| z | PASS | 0.212 | 0.212 | 0.000 | indistinguishable |
...
## Path Verdict: C
  ...
  - V1 fails on axis x (paper ratio=0.827+/-0.019, eff ratio=0.827+/-0.019). Inspect R22 formula in motion_control_law_eq17_7state.m R(2,2) assembly and Phase 6 derivation.
  - V2 fails on axis x (slope ratio paper=0.621, eff=0.621, R^2=0.981). Linearity in a_cov broken; suspect IF_var or (a_true+xi)^2.
  - V3 fails on axis x. Suspect xi(sigma2_n) scaling — check linearity of xi in sigma2_n (Phase 6 derivation).
  - V4 fails on axis x (max ACF dev=0.114, IF_var ratio=0.929). Inspect MA(2) closed-form coeffs vs lambda_c=0.
  - V1 fails on axis y (paper ratio=0.820+/-0.013, eff ratio=0.820+/-0.013). Inspect R22 formula in motion_control_law_eq17_7state.m R(2,2) assembly and Phase 6 derivation.
  - V2 fails on axis y (slope ratio paper=0.643, eff=0.643, R^2=0.986). Linearity in a_cov broken; suspect IF_var or (a_true+xi)^2.
  - V4 fails on axis y (max ACF dev=0.133, IF_var ratio=0.912). Inspect MA(2) closed-form coeffs vs lambda_c=0.
  - V1 fails on axis z (paper ratio=0.788+/-0.015, eff ratio=0.788+/-0.015). Inspect R22 formula in motion_control_law_eq17_7state.m R(2,2) assembly and Phase 6 derivation.
  - V2 fails on axis z (slope ratio paper=0.641, eff=0.641, R^2=0.988). Linearity in a_cov broken; suspect IF_var or (a_true+xi)^2.
