function simOut = temp_run_pure_sim_aprime2nd(config, opts)
%TEMP_RUN_PURE_SIM_APRIME2ND  Driver for the 1st-vs-2nd-order a' controller
%   (temp_motion_control_law_eq17_aprime2nd). Passes aprime_order / aprime_rate_beta
%   / aprime_Q_override / aprime_delta_P0 knobs. Original header follows.
%TEMP_RUN_PURE_SIM_CENTERED  Copy of run_pure_simulation with the is_4state
%   branch dispatched to temp_motion_control_law_eq17_aprime2nd (adds the
...
%             trajectory_generator, calc_thermal_force, calc_gamma_inv
    ...
    clear motion_control_law motion_control_law_23state ...
          ...
          temp_motion_control_law_eq17_aprime2nd ...
    ...
    % TEMP (2026-07-22): 1st-vs-2nd-order a' knobs passthrough.
    if isfield(config, 'aprime_order') && ~isempty(config.aprime_order)
        ctrl_const.aprime_order = config.aprime_order;
    end
    if isfield(config, 'aprime_rate_beta') && ~isempty(config.aprime_rate_beta)
        ctrl_const.aprime_rate_beta = config.aprime_rate_beta;
    end
    if isfield(config, 'aprime_Q_override') && ~isempty(config.aprime_Q_override)
        ctrl_const.aprime_Q_override = config.aprime_Q_override;
    end
    if isfield(config, 'aprime_Q_oracle_tv') && ~isempty(config.aprime_Q_oracle_tv)
        ctrl_const.aprime_Q_oracle_tv = config.aprime_Q_oracle_tv;
    end
    % TEMP (2026-07-22, Option 1): oracle a' feedforward + thermal-grounded Q.
    if isfield(config, 'aprime_ff_oracle') && ~isempty(config.aprime_ff_oracle)
        ctrl_const.aprime_ff_oracle = config.aprime_ff_oracle;
    end
    if isfield(config, 'aprime_curvature_state') && ~isempty(config.aprime_curvature_state)
        ctrl_const.aprime_curvature_state = config.aprime_curvature_state;
    end
    if isfield(config, 'aprime_delta_P0') && ~isempty(config.aprime_delta_P0)
        ctrl_const.aprime_delta_P0 = config.aprime_delta_P0;
    end
    ...
    for k = 1:N
        ...
        if is_4state
            if opts.collect_diag
                [f_d_k, ekf_k, diag_k] = temp_motion_control_law_eq17_aprime2nd( ...
                                    del_pd_k, pd_k, p_m_delayed, P, ctrl_const, a_override_k);
            else
                [f_d_k, ekf_k] = temp_motion_control_law_eq17_aprime2nd( ...
                                    del_pd_k, pd_k, p_m_delayed, P, ctrl_const, a_override_k);
            end
        elseif is_5state
            if opts.collect_diag
