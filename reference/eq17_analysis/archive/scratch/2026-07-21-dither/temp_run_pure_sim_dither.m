function simOut = temp_run_pure_sim_dither(config, opts)
%TEMP_RUN_PURE_SIM_DITHER  Copy of temp_run_pure_sim_centered with an optional
%   z-axis hold-window DITHER injection (opts.dither) to validate the
%   "dither restores slope observability" derivation (5state_aprime_coupdate.tex
%   §11-12). During the first hold [0, t_hold] and final hold [t3, T_sim] a
%   small sinusoid A*sin(2*pi*f*t) is added to the z-component of the desired
%   trajectory (both pd[k+1] and the matched del_pd[k] increment). Everything
%   else identical to temp_run_pure_sim_centered.
%   TEMP diagnostic (chat 2026-07-21); delete after the dither-pilot verification.
%
...
%             trajectory_generator, calc_thermal_force, calc_gamma_inv
    ...
    % TEMP (chat 2026-07-21): z-axis hold-window dither injection (§11-12).
    %   opts.dither = struct('on',bool,'A',um,'f',Hz)
    if ~isfield(opts, 'dither') || isempty(opts.dither)
        opts.dither = struct('on', false, 'A', 0, 'f', 0);
    end
    if ~isfield(opts.dither, 'on'); opts.dither.on = false; end
    if ~isfield(opts.dither, 'A');  opts.dither.A  = 0;     end
    if ~isfield(opts.dither, 'f');  opts.dither.f  = 0;     end
    ...
    % TEMP (chat 2026-07-21) dither hold-window bounds (§11-12): first hold
    % [0, t_hold], final hold (t3, T_sim]. t3 mirrors trajectory_generator phase
    % boundary (t_hold + t_descend + n_cycles/frequency).
    dith_t_hold = P.traj.t_hold;
    if isfield(P.traj, 't_descend_override') && P.traj.t_descend_override > 0
        dith_t_desc = P.traj.t_descend_override;
    else
        dith_t_desc = 1 / P.traj.frequency;
    end
    dith_t3 = dith_t_hold + dith_t_desc + P.traj.n_cycles / P.traj.frequency;
    dither_z = zeros(N, 1);      % injected d1 on pd[k+1], z-component [um]
    ...
    for k = 1:N
        ...
        % --- (a2) TEMP (chat 2026-07-21) z-axis hold-window dither (§11-12).
        %   trajectory_generator evaluates at t_next = t_now + Ts, so pd_kp1 is
        %   pd[k+1] at time tk1 = t_now + Ts and pd[k] is at tk = t_now. Each
        %   endpoint is gated on its OWN hold-window membership so pd and del_pd
        %   stay a matched pair across window boundaries (and consistent with the
        %   pd_for_ctrl unit-delay chain: d0 here == the previous step's d1).
        d1_dither = 0;
        if opts.dither.on
            tk1 = t_now + Ts;    % time of pd[k+1]
            tk  = t_now;         % time of pd[k]
            in_hold1 = (tk1 <= dith_t_hold) || (tk1 > dith_t3);
            in_hold0 = (tk  <= dith_t_hold) || (tk  > dith_t3);
            d1 = opts.dither.A * sin(2*pi*opts.dither.f * tk1) * double(in_hold1);
            d0 = opts.dither.A * sin(2*pi*opts.dither.f * tk ) * double(in_hold0);
            pd_kp1(3)   = pd_kp1(3)   + d1;
            del_pd_k(3) = del_pd_k(3) + (d1 - d0);
            d1_dither = d1;
        end
        ...
        ekf_out(k, :)  = ekf_k.';
        dither_z(k)    = d1_dither;      % TEMP (chat 2026-07-21) injected d1 on pd[k+1]
    ...
    simOut.dither_z = dither_z;      % TEMP (chat 2026-07-21) commanded z-dither d1 [um]
    simOut.meta = struct( ...
        'config',         config, ...
        'params_value',   P, ...
        'scheme',         opts.scheme, ...
