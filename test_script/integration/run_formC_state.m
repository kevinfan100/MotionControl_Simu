% STATUS: ACTIVE (state-writing driver) -- spec reference/eq17_analysis/derivation/formC_state_da.tex;
%          controller model/controller/motion_control_law_formC_state.m
% FORK OF test_script/integration/run_formB_ws.m @ 73fdc9d | PURPOSE: same
%   canonical scenario, seeds and metrics, controller swapped for the state
%   writing (slope read off the gain state, one disturbance state da) |
%   EXPIRES: T2/T3/T4 adoption decision | production changes do NOT follow.
%   Arms: 'A' = da locked at -1/9 (shape + quadrature floor, no estimation)
%         'B' = da free from 0    (the acceptance: da_hat -> -1/9, unfed)
function out = run_formC_state(opts, test_opts)
%RUN_FORMC_STATE  Canonical-scenario driver for the state-writing (unknown wall
...
%   See also: motion_control_law_formB_ws, run_5state_expgain
    ...
    assert(any(strcmpi(opts.arm, {'A', 'B'})), ...
           'run_formC_state:badArm', 'opts.arm must be ''A'' or ''B''.');
    ...
    assert(cfg.h_min / pc.R >= H_BAR_MIN_PRIOR - 1e-12, ...
           'run_formC_state:hMinBelowTruthDomain', ...
    ...
    assert(cfg.h_bottom / pc.R >= H_BAR_MIN_PRIOR, ...
           'run_formC_state:troughBelowTruthDomain', ...
    ...
    [~, ~, floor_a_env] = local_envelope_priors(env_lo, env_hi);
    ...
    ov.Pf_da_std  = opts.Pf_da_std;   % uninformative by decision (i)
    ov.ws0_perp   = 1;                % plane

    switch upper(opts.arm)
        case 'A'
            ov.da_init = -1/9;        % far-field anchor, PINNED
            ov.lock_da = true;
        case 'B'
            ov.da_init = 0;           % law as written; the anchor is NOT fed
            ov.lock_da = false;
        otherwise
    ...
    par_pkg = [];
    if opts.par_law
        par_pkg = local_parallel_law_package_state(env_lo, env_hi);
        ov.da_par         = par_pkg.da;
    ...
    % ------------------------------------------------------------------
    fprintf('=== Form C state-writing driver -- arm: %s ===\n', tag);
    ...
    fprintf('arm=%s  lock_da=%d  da_init=%+.5f  sqrt_Pda=%.4f  sqrt_Pw0=%.4f  y2_on=%d  a_cov=%.3f\n', ...
            upper(opts.arm), ov.lock_da, ov.da_init, ov.Pf_da_std, ov.Pf_w0_std, ...
            opts.y2_on, cfg.a_cov);
    fprintf('ENVELOPE on w_bar in [%.3f, %.3f]: shape floor %.5f\n', ...
            env_lo, env_hi, floor_a_env);
    if opts.par_law
        fprintf('PAR LAW (x/y): da %.4f  w0 %.4f  floor %.4f  sup %.3f%%\n', ...
                par_pkg.da, par_pkg.w0, par_pkg.floor_a, 100 * par_pkg.fit_sup);
    else
        fprintf('PAR LAW (x/y): OFF -- x/y run the perpendicular disturbance\n');
    end

    fprintf('%6s | %10s %10s %11s | %8s %8s | %4s\n', 'seed', ...
            'desc pk %', 'osc RMS %', 'hold mean %', 'da bud', 'P44 bud', 'NaN');
    ...
    fprintf('prior width sqrt(P[0]) (controller-reported, z): da %.4f  (declared %.4f)\n', ...
            runs{1}.P_b_out(1, AX_Z), opts.Pf_da_std);
    ...
    out.metrics = struct('rows', Mrows, ...
                         ...
                         'axis', AX_Z, 'prior_std_bp', opts.Pf_da_std);
    out.cfg     = cfg;
    out.opts    = opts;
    out.arm_tag = tag;
    out.seeds   = seeds;
...
%   Outputs are sqrt-P quantities (standard deviations), [-].
    ...
    assert(h_lo > 1 && h_hi > h_lo, 'run_formC_state:badEnvelope', ...
...
%   Outputs are sqrt-P quantities (standard deviations), [-].
    ...
    assert(abs(floor_a - max(abs(b_eff - B_SEED) ./ h)) < 1e-12, ...
           'run_formC_state:ampPriorIdentity', ...
           'amplitude prior identity violated (%.3e) -- check both formulas.', ...
           abs(floor_a - max(abs(b_eff - B_SEED) ./ h)));
end


function pkg = local_parallel_law_package_state(w_lo, w_hi)
%LOCAL_PARALLEL_LAW_PACKAGE_STATE  One-number parallel package for the state
%   writing. The wall-parallel truth needs its own disturbance because its two
%   asymptotic anchors sit further apart than the perpendicular pair:
%       da_state(w) = 1/b_state(w) - 1 ,   b_state = (1-a_true)^2 / a'_true
%   da_par is the RMS-relative optimum over the planned envelope (closed form,
%   b* = sum(b^2)/sum(b)); w0_par is the integration constant the level demands
%   at the envelope top. Offline truth evaluation at init only -- the run-time
%   loop stays c-free.
    h  = linspace(w_lo, w_hi, 4000).';
    ...
    ap = -dc ./ c.^2;
    b_state = (1 - a).^2 ./ ap;
    b_star  = sum(b_state.^2) / sum(b_state);
    pkg.da  = 1 / b_star - 1;
    % w0 that reproduces the true level at the envelope top:
    %   a = 1 - 1/((1+da)(w - w0))  =>  w0 = w - 1/((1+da)(1-a))
    pkg.w0  = h(end) - 1 / ((1 + pkg.da) * (1 - a(end)));
    a_law   = 1 - 1 ./ ((1 + pkg.da) * (h - pkg.w0));
    pkg.floor_a = max(abs(a_law - a));
    pkg.fit_sup = max(abs(b_state / b_star - 1));
end
...
%                     gain error the x/y filters cannot represent away
    ...
    assert(w_lo - LO_PROBE > 1, 'run_formC_state:parProbeBelowContact', ...
           'near-wall boundary probe w_bar = %.3f is not > 1 (c_para domain).', ...
           w_lo - LO_PROBE);
    assert(HI_PROBE > w_lo, 'run_formC_state:parProbeDegenerate', ...
...
%                     the three width pushforwards through dA/dtheta
    ...
    assert(lam > 0 && isfinite(lam), 'run_formC_state:badCellLam', ...
           'cell radius ratio lam = %.4f must be finite and positive.', lam);
    assert(w_lo - LO_PROBE > 1, 'run_formC_state:cellProbeBelowContact', ...
...
%   ratio must stay <= 1.
    ...
    assert(any(w_desc) && any(w_osc) && any(w_hold), ...
           'run_formC_state:emptyMetricWindow', ...
...
%   log schema extended by the p / w_s parameter rows.
    ...
    clear motion_control_law_formC_state trajectory_generator calc_thermal_force;
    ...
    for k = 1:N
        ...
        [f_d_k, ekf_k, diag_k] = motion_control_law_formC_state(del_pd_k, pd_k, ...
