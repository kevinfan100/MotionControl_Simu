...
%          controller model/controller/motion_control_law_formC_dist.m
...
function out = run_formC_dist(opts, test_opts)
...
%
%   out = run_formC_dist()                          % arm 'dist', house seeds
%   out = run_formC_dist(struct('arm', 'base'))     % the 4-state baseline
%
...
%   Saved to test_results/run_formC_dist_<arm_tag>.mat (gitignored).
%
%   TEST-LADDER COMPATIBILITY FORM (inherited):
%       simOut = run_formC_dist(cfg, opts)   % single seed, direct simOut
%
%   See also: motion_control_law_formC_dist, run_formC_state, run_formB_ws

    % ---- test-ladder compatibility form: run_formC_dist(cfg, test_opts) ----
    ...
    assert(cfg.h_min / pc.R >= H_BAR_MIN_PRIOR - 1e-12, ...
           'run_formC_dist:hMinBelowTruthDomain', ...
    ...
    assert(cfg.h_bottom / pc.R >= H_BAR_MIN_PRIOR, ...
           'run_formC_dist:troughBelowTruthDomain', ...
    ...
    switch lower(opts.arm)
        case 'base'
            ov.lock_da = true;        % derivation (a): 4 states, no parameter
        case 'dist'
            ov.lock_da = false;       % derivation (b): 5 states, Q55 = 0
        otherwise
    ...
    % ------------------------------------------------------------------
    fprintf('=== Form C ADDITIVE-disturbance driver -- arm: %s ===\n', tag);
    ...
    out_file = fullfile(out_dir, ['run_formC_dist_' tag '.mat']);
    save(out_file, 'out');
    fprintf('saved: %s\n', out_file);
end
...
%   planned envelope and the published truth curve.
    ...
    assert(w_hi > w_lo && w_lo - w0 > 0, 'run_formC_dist:badEnvelope', ...
...
%   ratio must stay <= 1.
    ...
    assert(any(w_desc) && any(w_osc) && any(w_hold), ...
           'run_formC_dist:emptyMetricWindow', ...
...
%   with the controller hard-dispatched to motion_control_law_formC_dist. The
...
%   P_b aliases the controller diag contract already provides.
    ...
    clear motion_control_law_formC_dist trajectory_generator calc_thermal_force;
    ...
    simOut.meta = struct('config', config, 'params_value', P, ...
                         'seed', seed, 'driver', 'run_formC_dist', ...
