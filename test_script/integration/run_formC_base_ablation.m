...
%          4-state baseline?
%            A0 nominal | A1 y2 OFF | A2 fdet OFF | A3 both OFF
...
%             plot_formC_base_ablation
    ...
    arms = struct( ...
        'tag',  {'A0', 'A1', 'A2', 'A3'}, ...
        'name', {'nominal', 'y2 OFF', 'fdet OFF', 'both OFF'}, ...
        'y2_off',   {false, true,  false, true}, ...
        'use_fdet', {true,  true,  false, false});
    n_arms = numel(arms);
    ...
    for a = 1:n_arms
        ...
        fprintf('\n########## ARM %s (%s): y2_off=%d use_fdet=%d ##########\n', ...
                arms(a).tag, arms(a).name, arms(a).y2_off, arms(a).use_fdet);
