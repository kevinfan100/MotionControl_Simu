...
% to reference/for_test/qr_verification_report.md.
...
if all([results.run_ok])
    ...
    fig_path = fullfile(project_root, 'reference', 'for_test', 'fig_qr_verification.png');
    saveas(fig, fig_path);
    close(fig);
    fprintf('\nFigure saved: %s\n', fig_path);
end
...
rep_path = fullfile(project_root, 'reference', 'for_test', 'qr_verification_report.md');
