% FORK OF test_script/scratch/run_aptrue_cmd_vs_est.m (2026-09-02) | PURPOSE: left = slope
%   read at the particle's TRUE height of the previous step (ap_known_at = 'true', oracle
%   position), right = slope at the filter's height ('est'); both exact + pred_mean2 +
%   app_known, same 10 seeds, both trajectories. PRE-REGISTERED: means equal within 0.001
%   (the start-point term reappears on the truth side with the same mean); spread of the
%   'true' arm may be larger. Also settles the 08-26 note (Euler, 'true' -0.021 vs 'cmd' -0.002).
%   | EXPIRES: with the parent | 產線改動不會自動跟上
function out = run_aptrue_true_vs_est(traj, seeds)
    ...
    O = {base, base};  O{1}.ap_known_at = 'true'; O{2}.ap_known_at = 'est';  TAG = {'left','right'};  LAB = {'slope@true','slope@est'};
    ...
    fprintf('[%s] paired hold true - est %+.5f (SEM %.5f)\n', traj, mean(d), std(d)/sqrt(nS));
    save(fullfile(od, sprintf('aptrue_true_vs_est_%s.mat', traj)), '-struct', 'out', '-v7.3');
    fprintf('[%s] saved aptrue_true_vs_est_%s.mat\n', traj, traj);
end
