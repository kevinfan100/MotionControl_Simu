% STATUS: ACTIVE (scratch) | PURPOSE: the a'_true@est arm with the row-4 known
%   increment = commanded displacement a_hat fbar_d (ctrl_const.pred_force_step) on
%   top of exact + pred_mean2, both trajectories, same 10 seeds as
%   run_aptrue_cmd_vs_est.m, paired against that run's slope@est arm (proxy
%   increment). PRE-REGISTERED hold (est - true): canon 0 +- 0.0003, meng 0 +- 0.0007.
%   Output: aptrue_force_step_<traj>.mat, aptrue_force_step.png (1x2 mean E(t)).
function run_aptrue_force_step(seeds)
    ...
    for it = 1:2
        ...
        o = struct('arm','best','ap_known',true,'ap_known_at','est','app_known',true, ...
                   'ctrl_const_override',struct('lock_b',true,'ws0_perp',ws0,'law_exact_step',true,'pred_mean2',true,'pred_force_step',true), ...
        ...
        fprintf('[%s force-step] health: min a_hat %.5f | NaN %d | E(0) %+.1e | mean2 sum/seed %+.5f\n', traj, min(AH(:)), sum(~isfinite(E(:))), mean(E(1,:)), mean(sum(M2,1)));
        fprintf('[%s] hold  est(proxy) %+.5f (SEM %.5f) -> est(force) %+.5f (SEM %.5f) | paired diff %+.5f (SEM %.5f)\n', traj, mean(po), std(po)/sqrt(nS), mean(pm), std(pm)/sqrt(nS), mean(pm-po), std(pm-po)/sqrt(nS));
        fprintf('[%s] osc   est(proxy) %+.5f -> est(force) %+.5f | hold sd across seeds %.5f -> %.5f\n', traj, mean(mean(Eo(m_o,:),1)), mean(mean(E(m_o,:),1)), mean(std(Eo(m_h,:),0,2)), mean(std(E(m_h,:),0,2)));
        RES.(traj) = struct('t', t, 'E', E, 'Eold', Eo, 'M2', M2, 'seeds', seeds);
        save(fullfile(od, sprintf('aptrue_force_step_%s.mat', traj)), '-struct', 'RES', traj);
    end
    ...
    for it = 1:2
        ...
        legend([h1 h2], {'slope@est, proxy increment', 'slope@est, commanded displacement'}, 'Location','northoutside','Orientation','horizontal','FontSize',10,'FontWeight','bold','Box','on');
    ...
    png = fullfile(od, 'aptrue_force_step.png'); exportgraphics(f, png, 'Resolution', 150); close(f); fprintf('wrote %s\n', png);
end
