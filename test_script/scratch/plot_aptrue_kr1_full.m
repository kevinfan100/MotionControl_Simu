% FORK OF test_script/scratch/plot_aptrue_cmd_vs_est.m (2026-09-03) | PURPOSE: 5x2 panel for
%   run_aptrue_kr1_full.m -- left kr1 (a'' (1-lc)^2 K31 R1), right kr1_full (a'' (1-lc) K31 R1),
%   both exact + slope@est + pred_mean2, same 10 seeds, one figure per trajectory
%   | EXPIRES: with the parent | 產線改動不會自動跟上
function out = plot_aptrue_kr1_full(traj, ylims)
    ...
    D0 = load(fullfile(od, sprintf('aptrue_kr1_full_%s.mat', traj)));
    ...
    TAG = {'left','right'};  COLT = {sprintf('   a''_{true} @est  + a''''(1-\\lambda_c)^2 K_{31} R_1  (%s)', traj), sprintf('   a''_{true} @est  + a''''(1-\\lambda_c) K_{31} R_1  (%s)', traj)};   % both arms: exact + pred_mean2
    S = cell(1,2);
    for a = 1:2
    ...
    png = fullfile(od, sprintf('aptrue_kr1_full_%s.png', traj));
