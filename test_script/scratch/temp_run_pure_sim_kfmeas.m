% FORK OF model/dual_track/run_pure_simulation.m @ pre-b5dbbc2 | PURPOSE: driver for kfmeas fork ('state'/'kfmeas' arms) | EXPIRES: C2 channel (block C) resolved | 產線改動不會自動跟上
function simOut = temp_run_pure_sim_kfmeas(config, opts)
%TEMP_RUN_PURE_SIM_KFMEAS  Copy of temp_run_pure_sim_centered with the is_4state
%   branch dispatched to temp_motion_control_law_eq17_4state_kfmeas (adds the
%   aprime_source='kfmeas' mode: a' as a KF state updated by the y2 Jacobian
%   H(2,5), 5state_aprime_kf_meas.tex). Supports 'state' and 'kfmeas' arms +
%   aprime_state_init_scale / freeze_level_kf passthroughs.
...
%             trajectory_generator, calc_thermal_force, calc_gamma_inv
    ...
    clear motion_control_law motion_control_law_23state ...
          ...
          temp_motion_control_law_eq17_4state_kfmeas ...
    ...
    % NEW (chat 2026-07-20, kfmeas): a'-state init scale + predict-only level freeze.
    if isfield(config, 'aprime_state_init_scale') && ~isempty(config.aprime_state_init_scale)
        ctrl_const.aprime_state_init_scale = config.aprime_state_init_scale;
    end
    % TEMP (chat 2026-07-22): C2 deterministic level channel (kf_meas.tex §9).
    for c2f = {'use_c2', 'c2_Fmin', 'c2_knu3', 'c2_r3_scale', 'c2_window_s'}
        if isfield(config, c2f{1}) && ~isempty(config.(c2f{1}))
            ctrl_const.(c2f{1}) = config.(c2f{1});
        end
    end
    if isfield(config, 'freeze_level_kf') && ~isempty(config.freeze_level_kf)
        ctrl_const.freeze_level_kf = config.freeze_level_kf;
    end
    ...
    if opts.collect_diag
        ...
        diag_log.a_hat_d           = zeros(N, 1);   % NEW (kfmeas): z slot-4 a_d posterior
    ...
    for k = 1:N
        ...
        if is_4state
            if opts.collect_diag
                [f_d_k, ekf_k, diag_k] = temp_motion_control_law_eq17_4state_kfmeas( ...
                                    del_pd_k, pd_k, p_m_delayed, P, ctrl_const, a_override_k);
            else
                [f_d_k, ekf_k] = temp_motion_control_law_eq17_4state_kfmeas( ...
                                    del_pd_k, pd_k, p_m_delayed, P, ctrl_const, a_override_k);
            end
        elseif is_5state
            if opts.collect_diag
        ...
        if opts.collect_diag
            ...
            if isfield(diag_k, 'a_hat_d'); diag_log.a_hat_d(k) = diag_k.a_hat_d; end
