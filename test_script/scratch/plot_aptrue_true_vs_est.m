...
function out = plot_aptrue_true_vs_est(traj, ylims)
    ...
    D0 = load(fullfile(od, sprintf('aptrue_true_vs_est_%s.mat', traj)));
    ...
    TAG = {'left','right'};  COLT = {sprintf('        a''_{true}  slope@true  (%s)', traj), sprintf('        a''_{true}  slope@est  (%s)', traj)};   % both arms: exact + pred_mean2
    S = cell(1,2);
    for a = 1:2
    ...
    png = fullfile(od, sprintf('aptrue_true_vs_est_%s.png', traj));
