% FORK OF model/controller/motion_control_law_5state_powerlaw.m @ ae41395 | PURPOSE: instrumented diag (9 knobs, P45/K2/H25/s_h/dx_r logs) | EXPIRES: powerlaw handoff open items resolved | 產線改動不會自動跟上
function [f_d, ekf_out, diag] = temp_mcl_powerlaw_diag(del_pd, pd, p_m, params, ctrl_const, a_ctrl_override)
...
%
%   [f_d, ekf_out]       = temp_mcl_powerlaw_diag(del_pd, pd, p_m, params, ctrl_const)
%   [f_d, ekf_out, diag] = temp_mcl_powerlaw_diag(...)
%   [f_d, ekf_out, diag] = temp_mcl_powerlaw_diag(..., ctrl_const, a_ctrl_override)
%
...
%             calc_correction_functions
    ...
    if has_override
        ...
        assert(numel(a_ctrl_override) == 3 && all(isfinite(a_ctrl_override)) && all(a_ctrl_override > 0), ...
               'temp_mcl_powerlaw_diag:badOverride', ...
    ...
    persistent p_fb_off      % scratch: 1 = break the p_hat -> s_h feedback path
    persistent r2_scale      % scratch: multiply R2 (colour de-rating of y2)
    persistent y2_decim      % scratch: only feed y2 every N steps (whitening)
    persistent h24_fix       % scratch: 1 = restore the dropped H(2,4) correction
    persistent y2_whiten     % scratch: 1 = feed the WHITENED a_xm increment
    persistent a_xm_km1      % scratch: a_xm[k-1] for the whitening difference
    persistent fe_use_fdet   % scratch: 1 = build F_e from the DETERMINISTIC force
    persistent fdet_km1 fdet_km2   % own history of the noise-free control mirror
    ...
    if isempty(initialized)
        ...
        p_fb_off  = get_field_default(ctrl_const, 'p_fb_off', 0);
        r2_scale  = get_field_default(ctrl_const, 'r2_scale', 1);
        y2_decim  = get_field_default(ctrl_const, 'y2_decim', 1);
        h24_fix   = get_field_default(ctrl_const, 'h24_fix', 0);
        % defaults synced to production (whitened y2, 0.025 p prior) so this
        % scratch fork cannot silently exercise the pre-fix behaviour.
        y2_whiten = get_field_default(ctrl_const, 'y2_whiten', 1);
        fe_use_fdet = get_field_default(ctrl_const, 'fe_use_fdet', 0);
        Pf_a_frac = get_field_default(ctrl_const, 'Pf_a_frac', 0.3);
        Pf_p_std  = get_field_default(ctrl_const, 'Pf_p_std', 0.025);
        ...
        a_init_honest = get_field_default(ctrl_const, 'a_init_honest', 0);
        a_init_asym   = get_field_default(ctrl_const, 'a_init_asym', 0);
        if a_init_asym > 0.5
            % scratch: seed from the FAR-FIELD REFLECTION ASYMPTOTE only --
            %   c_perp ~ 1 + (9/8)/h_bar ,  c_para ~ 1 + (9/16)/h_bar
            % the same published result that pins p = 1; needs h_bar_0 (wall
            % position, already assumed known) but NOT the full c(h_bar) curve.
            p0_init    = params.common.p0(:);
            h_init_um  = dot(p0_init, w_hat_n) - pz_wall;
            hb0        = max(h_init_um / R_radius, 1.001);
            a_h_init   = [a_nom_p/(1 + (9/16)/hb0); ...
                          a_nom_p/(1 + (9/16)/hb0); ...
                          a_nom_p/(1 + (9/8)/hb0)];
        elseif a_init_honest > 0.5
            % scratch: NO peek at c(h_bar) -- seed the far-field gain only
            a_h_init = [a_nom_p; a_nom_p; a_nom_p];
        elseif enable_wall && isfield(params, 'common') && isfield(params.common, 'p0')
            p0_init    = params.common.p0(:);
            h_init_um  = dot(p0_init, w_hat_n) - pz_wall;
        ...
        a_xm_km1 = a_h_init;   % prefill is consistent with a_xm[0] = a_h_init
    ...
    elseif d_delay == 1
        ...
        error('temp_mcl_powerlaw_diag:unsupportedDelay', ...
    ...
    % Deterministic mirror of the control law with the noise-driven feedback
    % removed (delta_x_m := 0) and running on its OWN history -> purely
    % trajectory-driven, hence EXOGENOUS. Used for F_e when fe_use_fdet = 1.
    % (mirrors motion_control_law_eq17_4state's f_det / fe_eval_fdet probe)
    if d_delay == 2
        sum_af_det = a_ctrl_km1 .* fdet_km1 + a_ctrl_km2 .* fdet_km2;
        F_dh_det   = 0;  % filled per axis below
    else
    ...
    a_prime_v  = zeros(3, 1);
    Q44_v      = zeros(3, 1);
    % --- scratch instrumentation (temp diag copy only) ---
    dg_P45 = zeros(3,1); dg_P44 = zeros(3,1); dg_P55 = zeros(3,1);
    dg_K25 = zeros(3,1); dg_S2  = zeros(3,1); dg_R2  = zeros(3,1);
    dg_H25 = zeros(3,1); dg_sh  = zeros(3,1); dg_sa  = zeros(3,1);
    dg_K25_via_P45 = zeros(3,1); dg_K25_via_P55H = zeros(3,1);
    dg_H24_drop    = zeros(3,1);   % the DROPPED O(dHbar) correction to H(2,4)
    dg_dA_y1 = zeros(3,1); dg_dA_y2 = zeros(3,1); dg_dA_pred = zeros(3,1);
    ...
    for ax = 1:3
        ...
        % scratch: p_fb_off = 1 breaks the p_hat -> s_h -> a_h-sweep feedback
        % (p_hat is still estimated, it just no longer drives the prediction).
        if p_fb_off > 0.5
            p_use = 1.0;
        else
            p_use = p_i;
        end
        [s_h, s_a, s_h_over_p] = local_slope(a_h_i, p_use, h_bar_d, a_nom_p, enable_wall);
        ...
        R1_i = sigma2_n_s(ax);
        IF_eff_i = if_eff_eval(IF_abc, C_dpmr, C_n, kBT, a_h_i, sigma2_n_s(ax));
        R2_intr  = amlpf_var_factor * K_var * IF_eff_i * (a_h_i + xi_per_axis(ax))^2;
        R2_i = r2_scale * (R2_intr + delay_R2_factor * Q_i(4, 4));
        % whitened branch: y2' = a_xm[k]-(1-a_cov)a_xm[k-1] = a_cov*u[k],
        % Var(a_cov*u) = a_cov^2*2*IF*(a+xi)^2 = R2_intr * a_cov*(2-a_cov)
        % (uses K_var = 2*a_cov/(2-a_cov)); no free parameter.
        R2_w = r2_scale * (a_cov * (2 - a_cov) * R2_intr ...
                           + a_cov^2 * delay_R2_factor * Q_i(4, 4));
        ...
        if fe_use_fdet > 0.5
            F_dh = F_dh_det(ax);          % exogenous regressor (fdet fix)
        elseif d_delay == 2
        ...
        % scratch: drift_off = 1 drops the (1-lc)*a'*delta_h3_hat term from the
        % a_h prediction (candidate source of the monotonic a_hat drift).
        if get_field_default(ctrl_const, 'drift_off', 0) > 0.5
            drift_term = 0;
        else
            drift_term = one_minus_lc * a_prime_i * x_curr(3);
        end
        x_pred = [x_curr(2); ...
                  x_curr(3); ...
                  lambda_c * x_curr(3); ...
                  x_curr(4) + s_h * Delta_hbar_d + drift_term; ...
                  x_curr(5)];
        dg_dA_pred(ax) = x_pred(4) - x_curr(4);
        ...
        K_dx_y1_v(ax) = K1(3);
        dg_dA_y1(ax) = K1(4) * innov1;

        % (b) y2 = a_xm observes a_h[k-d]; skipped when gated (KF ignores y_2)
        dg_sh(ax) = s_h;  dg_sa(ax) = s_a;  dg_R2(ax) = R2_i;
        dg_H24_drop(ax) = -Delta_Hbar_d * s_a;      % dropped H(2,4) correction

        y2_due = (mod(k_step, max(round(y2_decim), 1)) == 0);
        if ~gate_off(ax) && y2_due
            H25 = -Delta_Hbar_d * s_h_over_p;               % H(2,5)
            if h24_fix > 0.5
                H24 = 1 - Delta_Hbar_d * s_a;               % restored O(dHbar) term
            else
                H24 = 1;                                    % as derived (dropped)
            end
            H2  = [0 0 0 H24 H25];
            if y2_whiten > 0.5
                H2 = a_cov * H2;              % E[y2'] = a_cov * (H x)
                R2_use = R2_w;
                y2_val = a_meas(ax) - (1 - a_cov) * a_xm_km1(ax);
            else
                R2_use = R2_i;
                y2_val = a_meas(ax);
            end
            S2  = H2 * P_upd * H2' + R2_use;
            K2  = (P_upd * H2') / S2;
            if freeze_gain; K2(4:5) = 0; end
            dg_P45(ax) = P_upd(4,5); dg_P44(ax) = P_upd(4,4); dg_P55(ax) = P_upd(5,5);
            dg_H25(ax) = H25; dg_S2(ax) = S2; dg_K25(ax) = K2(5);
            dg_K25_via_P45(ax)  = P_upd(4,5) / S2;
            dg_K25_via_P55H(ax) = P_upd(5,5) * H25 / S2;
            innov2 = y2_val - H2 * x_upd;
            x_upd  = x_upd + K2 * innov2;
            ImKH2  = I5 - K2 * H2;
            P_upd  = ImKH2 * P_upd * ImKH2' + K2 * R2_use * K2';   % Joseph form
            P_upd  = 0.5 * (P_upd + P_upd');
            K_a_y2_v(ax)   = K2(4);
            dg_dA_y2(ax) = K2(4) * innov2;
    ...
    if nargout >= 3
        ...
        diag.dg_P45 = dg_P45; diag.dg_P44 = dg_P44; diag.dg_P55 = dg_P55;
        diag.dg_K25 = dg_K25; diag.dg_S2 = dg_S2;   diag.dg_R2 = dg_R2;
        diag.dg_H25 = dg_H25; diag.dg_sh = dg_sh;   diag.dg_sa = dg_sa;
        diag.dg_K25_via_P45 = dg_K25_via_P45;
        diag.dg_K25_via_P55H = dg_K25_via_P55H;
        diag.dg_H24_drop = dg_H24_drop;
        diag.dg_dA_y1 = dg_dA_y1; diag.dg_dA_y2 = dg_dA_y2; diag.dg_dA_pred = dg_dA_pred;
    end
end
...
%EMPTY_DIAG_POWERLAW  Zeroed diagnostic struct (driver collect_diag compatible).
    ...
    d.P_dx1             = zeros(3, 1);
    d.dg_P45 = zeros(3,1); d.dg_P44 = zeros(3,1); d.dg_P55 = zeros(3,1);
    d.dg_K25 = zeros(3,1); d.dg_S2  = zeros(3,1); d.dg_R2  = zeros(3,1);
    d.dg_H25 = zeros(3,1); d.dg_sh  = zeros(3,1); d.dg_sa  = zeros(3,1);
    d.dg_K25_via_P45 = zeros(3,1); d.dg_K25_via_P55H = zeros(3,1);
    d.dg_H24_drop = zeros(3,1);
    d.dg_dA_y1 = zeros(3,1); d.dg_dA_y2 = zeros(3,1); d.dg_dA_pred = zeros(3,1);
end
