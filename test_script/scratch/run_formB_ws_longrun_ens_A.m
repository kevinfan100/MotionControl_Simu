% run_formB_ws_longrun_ens_A.m -- PURPOSE: E1 arm A of the seed-11 self-lock
...
res_file = fullfile(proj, 'test_results', 'formB_longrun_ens_A.mat');
SEEDS = 5001:5006; INJ = 0.05; N_CYC = 32; AX = 3;
...
for i = 1:numel(SEEDS)
    ...
    fprintf('E1A seed %d: dev_end %+.2f  sqP %.2f  z_end %.2f  b_end %.4f\n', ...
            SEEDS(i), dev(end), sqP(end), E.z_end(i), E.b_end(i));
end
fprintf('E1A done: trapped(z>2) %d/%d\n', nnz(E.z_end > 2), numel(SEEDS));
