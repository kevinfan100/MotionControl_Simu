% FORK OF test_script/scratch/plot_formB_c3_cell5.m @ untracked-2026-08-03 | PURPOSE:
% the OPERATING question -- plant is the exact two-sphere truth for Rc = 5 um while the
% controller knows only the plane (anchors 9/8, 1, envelope priors 0.0157 / 0.0245, law
% origin at the nominal contact): what does a plane-anchored filter do when it meets a
% cell? Figure for formB_selected_figs.tex page 5 | EXPIRES: when the cell branch is
% chartered or dropped | 產線改動不會自動跟上
%
...
SEEDS_SHOWN  = [7 11];
PRIOR_WS     = 0.028;
CELL_RC      = 5;              % [um] cell radius -- PLANT ONLY, controller is blind

% what the free parameters would have to reach for the plane-anchored law to
% represent this cell (the offline cell fit, computed here for reference only)
TARGETS      = [1.5698, 1.7440, 1];
...
o = struct();
o.seeds = SEEDS_SHOWN;
o.plant_cell_Rc = CELL_RC;       % TRUE boundary = cell
% NO o.ctrl_cell_Rc -- the controller keeps every plane-derived anchor and prior
...
for target = SEEDS_SHOWN
    ...
    legend([ht he hx], {'a_{true} (cell)', 'a_{hat} (plane-anchored)', 'a_{wm} (raw)'}, ...
    ...
    legend([hs; ha], [lbl; {sprintf('cell needs %.2f / %.2f / 1', TARGETS(1), TARGETS(2))}], ...
    ...
    outp = fullfile(fig_dir, sprintf('formB_c3_cellblind5_seed%d.png', target));
    drawnow;
    ...
    fprintf('  b_hat final %.4f (cell needs %.4f), p_hat final %.4f (needs %.4f)\n', ...
            r.b_hat_out(end, AX_Z), TARGETS(1), r.p_hat_out(end, AX_Z), TARGETS(2));
end
