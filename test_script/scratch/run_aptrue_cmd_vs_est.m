% STATUS: ACTIVE (scratch) | PURPOSE: the two slope-reading points of the a'_true oracle
%   under the FULL estimator of derivation/0902_formC_aptrue_4state.tex (exact step +
%   pred_mean2 with exogenous a''), same 10 seeds, on both trajectories:
%     left  = slope read at the COMMANDED height   (ap_known_at = 'cmd')
%     right = slope read at the filter's height    (ap_known_at = 'est')
%   pred_mean2 carries no deterministic slope-evaluation group by design, so 'cmd'
%   keeps -a'' dw3_hat [Delta w_d + (1-lc) dw3_hat] uncompensated.
%   PRE-REGISTERED hold means (est - true): canon cmd ~ -0.013, est ~ +0.0005;
%   meng cmd ~ -0.02, est ~ 0 +- 0.001.
%   Output: test_results/apd_acov_meng/aptrue_cmd_vs_est_<traj>.mat (compact schema of
%   run_arms_seedtruth_pair.m, left/right), plotted by plot_aptrue_cmd_vs_est.m.
function out = run_aptrue_cmd_vs_est(traj, seeds)
    ...
    base = struct('arm','best','ap_known',true,'app_known',true, ...
    ...
    O = {base, base};  O{1}.ap_known_at = 'cmd';  O{2}.ap_known_at = 'est';  TAG = {'left','right'};  LAB = {'slope@cmd','slope@est'};
    ...
    fprintf('[%s] paired hold cmd - est %+.5f (SEM %.5f)\n', traj, mean(d), std(d)/sqrt(nS));
    save(fullfile(od, sprintf('aptrue_cmd_vs_est_%s.mat', traj)), '-struct', 'out', '-v7.3');
    fprintf('[%s] saved aptrue_cmd_vs_est_%s.mat\n', traj, traj);
end
