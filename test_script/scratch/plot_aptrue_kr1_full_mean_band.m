...
function plot_aptrue_kr1_full_mean_band(tag, lab)
    if nargin < 1 || isempty(tag); tag = 'kr1_full'; end
    if nargin < 2 || isempty(lab); lab = {'kr1  a''''(1-\lambda_c)^2 K_{31} R_1', 'kr1\_full  a''''(1-\lambda_c) K_{31} R_1'}; end
    ...
    for it = 1:2
        D = load(fullfile(od, sprintf('aptrue_%s_%s.mat', tag, TR{it})));  t = D.left.t;  nS = size(D.left.E, 2);
        ...
        legend(H, lab, 'Location','northoutside','Orientation','horizontal','FontSize',11,'FontWeight','bold','Box','on');
    ...
    png = fullfile(od, sprintf('aptrue_%s_mean_band.png', tag)); exportgraphics(f, png, 'Resolution', 150); close(f); fprintf('wrote %s\n', png);
end
