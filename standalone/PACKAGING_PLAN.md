...
| 11 | 場景組 | h50（5-seed 定量 PASS）+ h10（描述性）+ **osc1hz**（motion-test 1 Hz 振盪, 谷底 h_bar=1.2, 動態追蹤 + L2 近壁邊界）；三場景全 **T_sim=4s**（§9c 定案,取代原 ramp2p7） |
| 12 | Branch | D6 在 `feat/eq17-6state`；打包在 `pack/6state-standalone`（D6 後 merge 進來）；打包 commits 用 `pack(N):` 前綴 |

## 5. 三段式流程與狀態

> **⏩ 從這裡接續（2026-06-15, HEAD = `pack(9c)`, 待 push）— 8 部分 DONE + §9a + §9c 軌跡 DONE**
> **§9a（main_run 腳本化 + config overrides，6 旋鈕 + seed 自動隨機 + .mat 存檔）已完成。
> §9c-軌跡：場景組定案 {h50, h10, osc1hz} 全 T_sim=4s；osc1hz（motion-test 1 Hz osc_aggr）
> verbatim 移植回 trajectory_ref（type 1, 4-phase）取代 ramp2p7；軌跡參數鎖 config 不露出。
> 驗證全綠：gates_part2 G1/G2 osc bit-exact vs 母 / gates_part5 E3 + gates_part7 H3 osc L2 邊界
> （H3：bit-identical 到 k=2284 首次近壁,之後 a_hat_z 發散 ~100%）/ verify 三場景 / main_run osc 端到端 /
> 全 checkcode 0 / reviewer 待跑。走讀簽收待使用者確認。
> **下一步 = §9c-圖（使用者主導：要畫哪些圖/內容/檔名/存哪）→ 然後 §9b 交付結構 + §9d 剪臍帶。**
...
| 7 | 閉迴路等價 | **DONE**（走讀簽收待確認） | pack(8) | gates_part7（H1 h50/H2 h10 rounding-floor equiv；H3 osc1hz L2 邊界） |
...
1. **軌跡怎麼設定** — **DONE 2026-06-15（commit pack(9c)）**：場景組定案 = {h50, h10, osc1hz}，
   全 T_sim=4s。**osc1hz 取代 ramp2p7**（使用者選「直接取代」）：motion-test 1 Hz "osc_aggr" 軌跡
   verbatim 移植回 trajectory_ref（type 1, 4-phase hold/cosine-descend/1Hz-osc/hold；h_init=50,
   h_bottom=2.7=h̄1.2, amplitude=2.5→h̄∈[1.2,3.42], n_cycles=2, t_hold=0.5, t_descend=1.0）。
   **軌跡參數不露出**（h_init/h_bottom/amplitude/frequency/n_cycles 鎖 config，由 scenario 決定，
   不進 main_run 頂部）。驗證：gates_part2 G1/G2 osc bit-exact vs 母 repo、gates_part5 E3 + gates_part7
   H3 osc L2 邊界、verify_standalone 三場景全綠、main_run osc1hz 端到端跑通。標準 1 Hz 鎖死;
   其他頻率/振幅要改去 config。
2. **最後輸出的圖** — **仍待討論**：目前只有 fig1 gain / fig2 tracking（osc 用 is_dynamic 寬 y-lim 畫法）；
   要畫哪些、內容、數量、存到哪個資料夾、檔名規則 —— 使用者主導,下一步談這個。
