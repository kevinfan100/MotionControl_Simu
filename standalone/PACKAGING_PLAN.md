...
meas_noise_std = [0.00062; 0.00057; 0.00331];   % 各軸感測噪聲 std [x;y;z] (um)
thermal        = true;                          % 熱力 開/關
```
- **seed = [] → 隨機**：main_run 內 `if isempty(seed); seed = randi(2^31-1); end` 並 fprintf 印出。
- config 留在內部、不露出的：物理常數 (R/gamma_N/Ts/kBT)、d=2、wall θ/φ/pz、t_hold。
...
1. **軌跡怎麼設定**（h_init/h_bottom/amplitude/freq/cycles 要不要進頂部、osc 場景要不要加）
2. **輸出圖**（目前 fig1 gain / fig2 tracking；內容、要不要更多、存哪）
