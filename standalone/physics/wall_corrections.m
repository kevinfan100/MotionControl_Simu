function [c_para, c_perp, derivs] = wall_corrections(h_bar, want_derivs)
%WALL_CORRECTIONS Wall-effect correction coefficients c_para / c_perp (+ derivatives).
%
%   [c_para, c_perp] = wall_corrections(h_bar)
%   [c_para, c_perp, derivs] = wall_corrections(h_bar, true)
%   [c_para, c_perp, derivs] = wall_corrections(h_bar)   % nargout >= 3
%
%   Stokes-drag correction near a planar wall. SHARED by the plant
%   (gamma_inv, thermal_force) and the controller (K_h in Q55) -- a
%   deliberate perfect-model choice: controller and world use the same
%   correction functions by construction.
%
%   Inputs:
%       h_bar       - Normalized wall distance h/R. MUST be > 1.
%       want_derivs - (optional, default false) force derivs computation
%                     even when nargout < 3.
%
%   Outputs:
%       c_para - parallel correction (gamma_para = gamma_N * c_para)
%       c_perp - perpendicular correction (gamma_perp = gamma_N * c_perp)
%       derivs - struct: dc_para_dh, dc_perp_dh,
...
%
%   Both c -> 1 as h_bar -> inf; both grow as h_bar -> 1; c_perp > c_para.
%
%   Source: verbatim from model/wall_effect/calc_correction_functions.m
%   (packaging part 1; logic unchanged). Reference: Drag_MATLAB.pdf.

    if h_bar < 1
        error('wall_corrections:invalidInput', ...
