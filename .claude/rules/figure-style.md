...
# 報告與判準：運動段優先，hold 放最後（2026-09-07 使用者裁示）

- 除 a′_true／b_true 作弊臂的機制診斷外，任何臂的報告一律先給運動段（下坡／近壁窗：均值、σ_seed、最差瞬間），再振盪段，hold 最後。
- 預登記判準以運動段為主，hold 只當附帶；「hold 位準零」不等於「a 達標」。
- 來源：09-07 多次把 hold 零講成達標，繞了一整天沒抓到要處理的近壁暫態。
